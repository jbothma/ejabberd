...
\begin{description}
  ...
  The default value is to not use STARTTLS: \term{false}. The certificate is only
 checked for matching the domain with \term{required\_trusted}.
...
IMPORTANT: Note that it is important to restrict the information the LDAP user
used by \ejabberd{} has access to in your LDAP server configuration. As always, connecting to other databases might reveal more information than
intended. For example, the ejabberd LDAP user should only have authentication
access to passwords.
The ejabberd LDAP user should not have read or search access to passwords or any
other private attributes or unnecessary directories.
...
One module can be started per cluster node. Rooms are
...
IMPORTANT: Note that it is important to restrict the information the LDAP user
used by \ejabberd{} has access to in your LDAP server configuration. vCards
and especially User Directory searches might reveal more information than
intended. For example, the ejabberd LDAP user should only have authentication
access to passwords.
The ejabberd LDAP user should not have read or search access to passwords or any
other private attributes or unnecessary directories.
...
\begin{description}
\hostitem{vjud}
\iqdiscitem{\ns{vcard-temp}} \term{jabber:iq:search} queries are always performed
according to the \term{\{iqdisc, parallel\}} configuration.
\titem{\{search, true|false\}}\ind{options!search}This option specifies whether the search
...
\titem{\{ldap\_vcard\_map, [ \{Name, Pattern, LDAPattributes\}, ...]\}} \ind{options!ldap\_vcard\_map}
  ...
  The mapping \term{\{"PHOTO", Pattern, [AttrName]\}} is a special case, where Pattern
  is ignored and always treated as \term{"\%s"} and the photo binary is
  base64-encoded.
  The default is:
\begin{verbatim}
[{"NICKNAME", "%u", []},
